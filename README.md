...
|---------|-------|--------|
| v1.3.3 | 2026-09-21 | **Cierre del ciclo de context engineering** (auditoría del 2026-09-18 que había quedado sin commitear). Fix: dominio `https://codetlon.com` (sin `.ar`, NXDOMAIN) hardcodeado en canonical/OG/JSON-LD de ~10 `page.tsx` — corregido a `codetlon.com.ar`, ahora todos importan `SITE_URL` de `constants.ts` en vez de repetir el literal. Fix deps: `npm audit fix` resuelve el 1 HIGH activo (`browserslist`, transitiva vía `shadcn`) — `npm audit` queda en 0 vulnerabilidades. Docs: se agregan `AGENTS.md` + `.ai/context/` (9 archivos) como capa de contexto para agentes de IA, incluyendo el veredicto explícito de que ni RAG ni un subagente de mantenimiento aplican hoy (corpus chico y estático, mantenedor único). Build verde. |
| v1.3.2 | 2026-07-11 | Baseline de seguridad + performance percibida: `next.config.mjs` suma `Strict-Transport-Security` y sube `X-Frame-Options` a `DENY` (checklist `security-owasp.md` §5); resto de headers (`nosniff`, `Referrer-Policy`, `Permissions-Policy`) ya estaban. CORS/RLS no aplican (form solo Resend vía Server Action, sin route handlers ni Supabase). **Progressive loading**: nuevo `Skeleton` (shadcn) + `PageSkeleton` compartido, con `loading.tsx` en cada ruta top-level (home, servicios, contacto, nosotros, proceso, gracias, privacidad, terminos). **Error boundaries**: `ErrorFallback` de marca (logo + botón peach) con `error.tsx` por ruta + `global-error.tsx` raíz (self-contained, sin depender del layout que puede haber roto). Build verde 23/23. |
